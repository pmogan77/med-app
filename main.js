...
var head, upperBack, lowerBack, neck, shoulder, upperleftArm, rightElbow, leftElbow, upperrightArm, lowerleftArm, lowerrightArm, chest, stomach, groin, buttox, leftThigh, rightThigh, leftLowerLeg, rightLowerLeg, leftKnee, rightKnee, leftHand, rightHand, leftFoot, rightFoot;
...
var movementSpeechImpairmentForm = document.getElementById('movement/ speech impairment');
...
function init() {
    ...
    shoulder = new THREE.BoxGeometry( 0.5, .2, 0.2 );
    rightElbow = new THREE.BoxGeometry( .12, 0.12, 0.23 );
    leftElbow = new THREE.BoxGeometry( .12, 0.12, 0.2 );
    ...
    limbMaterial = new THREE.MeshPhongMaterial( { color: materialColor, specular: 0x009900, opacity: 0.3, transparent: true } );
    ...
    meshleftElbow.position.set(.4, 1.4, 0.18);
    meshrigtElbow.position.set(-.4, 1.4, 0.18);
    meshupperleftArm.position.set(0.25, 1.65, 0.15);
    meshupperrightArm.position.set(-0.25, 1.65, 0.15);
    meshlowerleftArm.position.set(.48, 1.2, 0.18);
    meshlowerrightArm.position.set(-.48, 1.2, 0.18);
    ...
    meshleftThigh.position.set(.1, .9, 0.2);
    meshrightThigh.position.set(-.1, .9, 0.2);
    meshleftKnee.position.set(0.2, .6, 0.2);
    meshrightKnee.position.set(-0.2, .6, 0.2);
    meshleftLowerLeg.position.set(0.17, .4, 0.16);
    meshrightLowerLeg.position.set(-0.17, .4, 0.16);
    ...
    group.add(meshhead, meshbuttox, meshupperBack, meshlowerBack, meshneck, meshshoulder, meshupperleftArm, meshrigtElbow, meshleftElbow, meshupperrightArm, meshlowerleftArm, meshlowerrightArm, meshchest, meshstomach, meshgroin, meshleftThigh, meshrightThigh, meshleftLowerLeg, meshrightLowerLeg, meshrightKnee, meshleftKnee, meshleftHand, meshrightHand, meshleftFoot, meshrightFoot);
    scene.add( group );
...
function onDocumentMouseMove( event ) {
	...
	if(isChildOf(modal, event.target)&&event.target.getAttribute("for") == "symptom"){
...
function correctSymptoms(limbName){
	...
	}else if(limbName.toString().includes('leg') || limbName.toString().includes('knee')||limbName.toString().includes('thigh')){
